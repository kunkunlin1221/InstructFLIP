...
class FeatureGeneratorMADDG(nn.Module):
...
class FeatureEmbedderMADDG(nn.Module):
    def __init__(self):
        super(FeatureEmbedderMADDG, self).__init__()
...
class FeatureGeneratorResNet18(nn.Module):
    def __init__(self, in_channels=3):
        super().__init__()
        ...
        if in_channels == 3:
            ...
            self.conv1 = nn.Conv2d(in_channels, model_resnet.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
...
class FeatureEmbedderResNet18(nn.Module):
    def __init__(self):
        super(FeatureEmbedderResNet18, self).__init__()
...
class DG(nn.Module):
    def __init__(self, model_name, in_channels=3):
        ...
        if model_name == "resnet18":
            self.backbone = FeatureGeneratorResNet18(in_channels)
            self.embedder = FeatureEmbedderResNet18()
        elif model_name == "maddg":
            self.backbone = FeatureGeneratorMADDG(in_channels)
            self.embedder = FeatureEmbedderMADDG()
        else:
            print("Wrong Name!")
        self.classifier = Classifier()

    def forward(self, input, norm_flag: bool = True):
