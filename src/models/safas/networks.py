...
class ResNet(AbstractResNet):
    ...
    def update_weight_d2(self, alpha=0.99):
        ...
        self.fc0.weight.data = (
            normalizer(alpha * beta0 + (1 - alpha) * beta1) * torch.norm(self.fc0.weight.data)
        ).unsqueeze(0)
        self.fc1.weight.data = (
            normalizer(alpha * beta1 + (1 - alpha) * beta0) * torch.norm(self.fc1.weight.data)
        ).unsqueeze(0)
    ...
    def update_weight_v4(self, alpha=0.99):
        ...
        self.fc0.weight.data = (
            normalizer(alpha * beta0 + (1 - alpha) * beta0_fr) * torch.norm(self.fc0.weight.data)
        ).unsqueeze(0)
        self.fc1.weight.data = (
            normalizer(alpha * beta1 + (1 - alpha) * beta1_fr) * torch.norm(self.fc1.weight.data)
        ).unsqueeze(0)
        self.fc2.weight.data = (
            normalizer(alpha * beta2 + (1 - alpha) * beta2_fr) * torch.norm(self.fc2.weight.data)
        ).unsqueeze(0)
