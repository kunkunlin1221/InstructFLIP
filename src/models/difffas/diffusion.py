...
from models.losses import discretized_gaussian_log_likelihood, normal_kl
    ...
    with torch.no_grad():
        ...
        with tqdm.tqdm(zip(reversed(seq), reversed(seq_next)), desc="DDIM Sampling", total=len(seq)) as progress_bar:
            for i, j in progress_bar:
                ...
                c1 = model_kwargs.get("eta", 0) * ((1 - at / at_next) * (1 - at_next) / (1 - at)).sqrt()
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if x_cond is None:
            ...
            if use_pair_flag == True:
                model_output, cond_output, uncond_output = model.forward_with_cond_scale(
                    x=torch.cat([x, target_pose], 1),
                    encoder=encoder,
    ...
    def _vb_terms_bpd(self, model, encoder, x_start, x_t, t, clip_denoised=True, model_kwargs=None):
        """
        ...
        out = self.p_mean_variance(model, encoder, x_t, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs)
    ...
    def training_losses(
        self, model, encoder, x_start, cond_input, t, betas, prob, means_size, var_size, noise=None, **model_kwargs
    ):
        """
