...
class RGBInstructDataset(RestoreImageMixin, Dataset):
    color_channel = "rgb"
    ...
    def __init__(
        ...
        self.style_keys = ['illumination condition', 'enviroment', 'camera quality']
    ...
    def __getitem__(self, ind) -> Dict[str, Any]:
        ...
        out['content_output'] = "No GT"

        style_key = choice(self.style_keys)
        out['style_input'] = self.qa_dict[style_key]['question']
        out['style_output'] = "No GT"

        out['style_inputs'] = [self.qa_dict[key]['question'] for key in self.style_keys]
        out['style_outputs'] = ["No GT"] * len(self.style_keys)
